...
import { FindOneFirst, ForPrice, ForRoom } from 'api'
...
// 请求list数据
export const toSecondList = createAsyncThunk('获取第一次数据接口', async () => {
	const res = await FindOneFirst()
...
export const forPrice = createAsyncThunk(
	'根据价格获取数据接口',
	async (datas: any) => {
		const res = await ForPrice(datas)
...
export const secondSlice = createSlice({
	...
	extraReducers: {
		// 第一次请求数据接口成功返回函数
		[toSecondList.fulfilled.type]: (state, { payload }) => {
			// console.log(payload)
...
// 抛出同步reducers
export const { GET_LIST } = secondSlice.actions
